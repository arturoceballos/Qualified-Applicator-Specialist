...
function breadcrumbs() {
	...
	$homeText = esc_attr__('Home', 'cmsmasters');
	...
	echo '<span>' . esc_html__('You are here: ', 'cmsmasters') . '</span>' . "\n\t\t" . 
	'<a href="' . esc_url($homeLink) . '" class="cms_home">' . $homeText . '</a>' . $sep;
	...
	if (is_single()) {
		...
		echo '<span>' . esc_html__('Search Results for', 'cmsmasters') . "</span>: '" . get_search_query() . "'";
		...
		echo '<span>' . $tax_cat->name . '</span>';
	} else {
		echo '<span>' . esc_html__('No Breadcrumbs', 'cmsmasters') . '</span>';
	}
}
