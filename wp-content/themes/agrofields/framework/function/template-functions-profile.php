		...
		if ($template_type == 'page') {
			...
			if ($cmsms_option[CMSMS_SHORTNAME . '_profile_post_cat']) {
				$out = '<div class="profile_details_item">' . 
					'<div class="profile_details_item_title">' . esc_html__('Categories', 'cmsmasters') . ':' . '</div>' . 
...
function cmsms_profile_comments($template_type = 'page', $show = true) {
	if (comments_open()) {
		if ($template_type == 'page') {
			$out = '<a class="cmsms_profile_comments cmsms_theme_icon_comment" href="' . esc_url(get_comments_link()) . '" title="' . esc_attr__('Comment on', 'cmsmasters') . ' ' . get_the_title() . '">' . get_comments_number() . '</a>';
		} elseif ($template_type == 'post') {
			$cmsms_option = cmsms_get_global_options();
			$out = '';
			...
			if ($cmsms_option[CMSMS_SHORTNAME . '_profile_post_comment']) {
				$out = '<div class="profile_details_item">' . 
					'<div class="profile_details_item_title">' . esc_html__('Comments', 'cmsmasters') . ':' . '</div>' . 
					'<div class="profile_details_item_desc">' . 
						'<a class="cmsms_profile_comments cmsms_theme_icon_comment" href="' . esc_url(get_comments_link()) . '" title="' . esc_attr__('Comment on', 'cmsmasters') . ' ' . get_the_title() . '">' . get_comments_number() . '</a>' . 
					'</div>' . 
				'</div>';
			}
		}
		...
		$out = '<div class="profile_social_icons">';
			...
			$out .= '<ul class="profile_social_icons_list">';
				...
				foreach($social_icons as $social_icon) {
					...
					$out .= '<li>' . 
						'<a href="' . esc_url($social_icon_link) . '" class="' . $social_icon_icon . '" title="' . esc_attr($social_icon_title) . '"' . (($social_icon_target == 'true') ? ' target="_blank"' : '') . '></a>' . 
					'</li>';
				}
...
function cmsms_profile_more($cmsms_id, $show = true) {
	...
	if ($cmsms_profile_read_more == '') {
		$cmsms_profile_read_more = esc_attr__('Profile', 'cmsmasters');
	}
	
	
	$out = '<a class="button cmsms_profile_read_more" href="' . esc_url(get_permalink($cmsms_id)) . '">' . $cmsms_profile_read_more . '</a>';
