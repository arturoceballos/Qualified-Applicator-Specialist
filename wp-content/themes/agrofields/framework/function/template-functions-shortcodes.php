...
function cmsms_slider_post_date($type = 'post', $show = true) {
	if ($type == 'post') {
		$out = $out = '<abbr class="published cmsms_slider_post_date" title="' . esc_attr(get_the_date()) . '">' . 
				'<span class="cmsms_day">' . get_the_date('d') . '</span>' . 
				'<span class="cmsms_mon">' . get_the_date('M') . '</span>' . 
			'</abbr>' . 
...
function cmsms_slider_post_comments($type = 'post', $show = true) {
	if (comments_open()) {
		if ($type == 'post') {
			$out = '<a class="cmsms_slider_post_comments cmsms_theme_icon_comment" href="' . esc_url(get_comments_link()) . '" title="' . esc_attr__('Comment on', 'cmsmasters') . ' ' . get_the_title() . '">' . get_comments_number() . '</a>';
		} elseif ($type == 'project') {
			$out = '<a class="cmsms_slider_project_comments cmsms_theme_icon_comment" href="' . esc_url(get_comments_link()) . '" title="' . esc_attr__('Comment on', 'cmsmasters') . ' ' . get_the_title() . '">' . get_comments_number() . '</a>';
		}
