...
<article id="post-<?php the_ID(); ?>" <?php post_class($columns_num); ?>>
	...
	echo '<div class="pl_content">' . "\n" .
		...
		if ($cmsms_profile_subtitle != '') {
			echo '<p class="pl_subtitle">' . esc_html($cmsms_profile_subtitle) . '</p>' . "\n";
		}

		if ($cmsms_profile_details_title != '') {
			echo '<br>'.'<p class="pl_subtitle"><a href="mailto:'.esc_html($cmsms_profile_details_title) .'?Subject=Hello%20QAS" >' . esc_html($cmsms_profile_details_title) . '</a></p>' . "\n";
		}
