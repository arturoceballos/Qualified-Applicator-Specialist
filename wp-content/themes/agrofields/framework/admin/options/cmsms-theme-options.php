...
global $custom_meta_fields, 
	...
	$custom_campaign_meta_fields, 
	$custom_donation_meta_fields;
...
function cmsms_admin_enqueue_scripts($hook) {
	if ( 
		...
		wp_localize_script('cmsms_theme_options_js', 'cmsms_options', array( 
			'create_gallery' => 	esc_attr__('Create Gallery', 'cmsmasters'), 
			'select_format' => 		esc_attr__('Please select the format.', 'cmsmasters'), 
			'link_exists' => 		esc_html__('Link with this format already exists.', 'cmsmasters'), 
			'want_remove' => 		esc_html__('Do you realy want to remove this item?', 'cmsmasters'), 
...
function show_cmsms_meta_box() {
	...
	foreach ($custom_all_meta_fields as $field) {
		...
		case 'select_sidebar':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Default Sidebar', 'cmsmasters') . '</option>';
		...
		case 'select_slider':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Slider', 'cmsmasters') . '</option>';
		...
		case 'select_post_categ':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Blog Category', 'cmsmasters') . '</option>';
		...
		case 'select_project_cat':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Project Category', 'cmsmasters') . '</option>';
		...
		case 'select_pl_categ':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Profile Category', 'cmsmasters') . '</option>';
		...
		case 'rgba':
			...
			$std = explode('|', $field['std']);
			
			
			echo '<input type="text" id="' . $field['id'] . '_0" name="' . $field['id'] . '[0]" value="' . $meta[0] . '" class="my-color-field cmsms-rgba" data-default-color="' . $std[0] . '" />' . 
			'<div class="cl"></div>' . 
			'<input type="range" name="' . $field['id'] . '[1]" id="' . $field['id'] . '_1" value="' . (isset($meta[1]) ? $meta[1] : 0) . '" size="30" min="0" max="100" step="1" />' . 
			'<input type="text" name="' . $field['id'] . '_number" id="' . $field['id'] . '_number" value="' . (isset($meta[1]) ? $meta[1] : 0) . '" size="5" readonly="readonly" />' . 
			'<span class="description">' . $field['desc'] . '</span>' . 
			'<script type="text/javascript"> ' . 
				'jQuery(document).ready(function () { ' . 
					'(function ($) { ' . 
						"$('#" . $field['id'] . "_0').wpColorPicker( { " . 
							"palettes : ['#000000', '#ffffff', '#4ecdc4', '#ff6b6b', '#556270', '#aed957', '#707070', '#3d3d3d'] " . 
						'} ); ' . 
						"$('#" . $field['id'] . "_1').bind('change', function () { " . 
							"$('#" . $field['id'] . "_number').val($(this).val()); " . 
						'} ); ' . 
					'} )(jQuery); ' . 
				'} ); ' . 
		...
		case 'repeatable_link':
			...
			echo '<div class="ovh">' . 
				'<div class="fl"><strong>' . esc_html__('Title', 'cmsmasters') . '</strong></div>' . 
				'<div class="fl"><strong>' . esc_html__('Link', 'cmsmasters') . '</strong></div>' . 
			'</div>' . 
			...
			'<select name="' . $field['id'] . '-select" id="' . $field['id'] . '-select">' . 
				'<optgroup label="' . __('Blank Field', 'cmsmasters') . '">' . 
					'<option value="">' . esc_html__('Select Link', 'cmsmasters') . '</option>' . 
				'</optgroup>' . 
		...
		case 'repeatable_multiple':
			echo '<div class="ovh">' . 
				'<div class="fl"><strong>' . esc_html__('Title', 'cmsmasters') . '</strong></div>' . 
				'<div class="fl"><strong>' . esc_html__('Values', 'cmsmasters') . '</strong></div>' . 
			'</div>' . 
			'<ul id="' . $field['id'] . '-repeatable" class="custom_repeatable">';
		...
		case 'repeatable_media':
			echo '<select name="' . $field['id'] . '-select" id="' . $field['id'] . '-select">' . 
				'<option value="">' . esc_html__('Select Format', 'cmsmasters') . ' &nbsp;</option>';
				...
				foreach ($ids as $id) {
					if (isset($id[0]) && isset($id[1])) {
						echo '<li class="cmsms_gallery_item">' . 
							...
							'<a href="#" class="cmsms_gallery_cancel admin-icon-remove" title="' . esc_attr__('Remove', 'cmsmasters') . '"></a>' . 
						'</li>';
					}
				}
			}
