...
function register_css_styles() {
	if (!is_admin()) {
		...
		wp_register_style('theme-icons-set', get_template_directory_uri() . '/css/cmsms-theme-icons-set.css', array(), '1.0.0', 'screen');
		...
		wp_enqueue_style('theme-icons-set');
...
function register_js_scripts() {
	if (!is_admin()) {
		...
		if (shortcode_exists('cmsms_google_map_markers')) {
...
function register_admin_js_scripts() {
	...
	wp_localize_script('composer-shortcodes-extend', 'composer_shortcodes_extend', array( 
		// 'sc_name_field_custom_bg_color' => 	esc_attr__('Background Color', 'cmsmasters') 
	));
...
function cmsms_custom_mega_munu_item_output($args) {
	...
	if (($shcd_depth > 0 && !empty($shcd_item->highlight) && !empty($shcd_item->color)) || (!empty($shcd_item->color))) {
	...
	if ($shcd_depth == 0) {
		...
		$item_output .= '></span><span class="nav_bg_hov"';
...
function cmsms_fallback_menu($args) {
	...
	wp_list_pages(array( 
		'title_li' => '', 
		'link_before' => '<span class="nav_bg_clr"></span><span class="nav_bg_hov"></span><span>', 
		'link_after' => '</span>' 
	));
...
function cmsms_theme_bg_styles() {
	...
	if (is_numeric($cmsms_def_bg_img[0])) {
		$cmsms_def_bg_img_url = wp_get_attachment_image_src((int) $cmsms_def_bg_img[0], 'full');
	}
	...
	if ( 
		...
		if (is_numeric($cmsms_bg_img[0])) {
			$cmsms_bg_img_url = wp_get_attachment_image_src((int) $cmsms_bg_img[0], 'full');
		}
	}
	...
	body {
		...
		background-image : " . (($cmsms_bg_img_enable == 'true' && is_array($cmsms_bg_img)) ? 'url(' . ((is_numeric($cmsms_bg_img[0])) ? $cmsms_bg_img_url[0] : $cmsms_bg_img[1]) . ')' : 'none') . ";
	...
	body {
		...
		background-image : " . (($cmsms_option[CMSMS_SHORTNAME . '_bg_img_enable'] && !empty($cmsms_option[CMSMS_SHORTNAME . '_bg_img'])) ? 'url(' . ((is_numeric($cmsms_def_bg_img[0])) ? $cmsms_def_bg_img_url[0] : $cmsms_def_bg_img[1]) . ')' : 'none') . ";
